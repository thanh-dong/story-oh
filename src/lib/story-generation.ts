...
export function validateGenerateRequest(
  body: unknown
): { valid: true; data: GenerateStoryRequest } | { valid: false; error: string } {
...
export function validateStoryTree(tree: unknown): tree is StoryTree {
...
export async function generateStory(
...
): Promise<{ ok: true; data: GenerationResult } | { ok: false; error: string; status: number }> {
  ...
  if (!llmResponse.ok) {
    ...
    return { ok: false, error: `AI provider returned ${llmResponse.status}: ${text}`, status: 500 };
  }
  ...
  if (typeof result.title !== "string" || typeof result.summary !== "string") {
    return { ok: false, error: "AI response missing title or summary.", status: 500 };
  }

  if (!validateStoryTree(result.story_tree)) {
    return { ok: false, error: "AI generated an invalid story structure. Please try again.", status: 500 };
  }
  ...
  return {
    ...
    data: {
      ...
      age_range: result.age_range || req.audienceAge,
